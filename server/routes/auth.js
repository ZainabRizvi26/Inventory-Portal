const router = require('express').Router(); const jwt = require('jsonwebtoken'); const User = require('../models/User'); const AdminCode = require('../models/AdminCode');
const { requireAuth } = require('../middleware/auth');
const tokenFor = (user) => jwt.sign({ id: user._id, role: user.role, isSuperAdmin: !!user.isSuperAdmin }, process.env.JWT_SECRET, { expiresIn: '7d' });
router.post('/register', async (req, res) => { try { const { email, username, fullName, password, phone, dob, location, role, adminCode } = req.body; if (!email || !password) return res.status(400).json({ message: 'Email and password are required.' }); if (await User.findOne({ $or: [{ email }, ...(username ? [{ username }] : [])] })) return res.status(409).json({ message: 'An account with that email or username already exists.' }); let finalRole = 'user'; if (role === 'admin') { const stored = await AdminCode.findOne({ key: 'admin-signup-code' }); if (!stored || !adminCode || adminCode !== stored.code) return res.status(403).json({ message: 'Invalid admin code. Admin access denied.' }); finalRole = 'admin'; } const user = await User.create({ email, username, fullName, password, phone, dob, location, role: finalRole, lastSeen: new Date() }); res.status(201).json({ token: tokenFor(user), user: { id: user._id, fullName: user.fullName, role: user.role, isSuperAdmin: user.isSuperAdmin, location: user.location } }); } catch { res.status(400).json({ message: 'Unable to create the account. Please check your details and try again.' }); } });
router.post('/login', async (req, res) => { try { const { identifier, password, role } = req.body; if (!identifier || !password) return res.status(400).json({ message: 'Username/email and password are required.' }); const user = await User.findOne({ $or: [{ email: identifier.toLowerCase() }, { username: identifier }] }); if (!user || !(await user.comparePassword(password)) || (role && user.role !== role)) return res.status(401).json({ message: 'Invalid credentials or portal type.' }); user.lastSeen = new Date(); await user.save(); res.json({ token: tokenFor(user), user: { id: user._id, fullName: user.fullName, role: user.role, isSuperAdmin: user.isSuperAdmin, location: user.location } }); } catch { res.status(500).json({ message: 'Unable to sign in.' }); } });
